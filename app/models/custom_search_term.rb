class CustomSearchTerm
	attr :where_clause, :where_args, :order
	...
	def build_for_email_search(search_term)
		name = extract_name(search_term)

		@where_clause << case_insensitive_search(:first_name)
		@where_args[:first_name] = starts_with(name)

		@where_clause << " OR #{case_insensitive_search(:last_name)}"
		@where_args[:last_name] = starts_with(name)

		@where_clause << " OR #{case_insensitive_search(:email)}"
		@where_args[:email] = search_term

		# Beware of sql injection
		#@order = "email = #{search_term}, last_name asc"
